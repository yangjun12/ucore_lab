...
vfs_get_root(const char *devname, struct inode **node_store) {
	cprintf("****************vfs_get_root ( a device name) back an appropriate inode\n");
    assert(devname != NULL);
    int ret = -E_NO_DEV;
    if (!list_empty(&vdev_list)) {
        lock_vdev_list();
...
vfs_get_devname(struct fs *fs) {
	cprintf("****************vfs_get_root (filesystem) name of the device it's mounted on \n");
    assert(fs != NULL);
    list_entry_t *list = &vdev_list, *le = list;
    while ((le = list_next(le)) != list) {
        vfs_dev_t *vdev = le2vdev(le, vdev_link);
...
check_devname_conflict(const char *devname) {
	cprintf("****************check_devname_conflict  which has the same name \n");
    list_entry_t *list = &vdev_list, *le = list;
    while ((le = list_next(le)) != list) {
        vfs_dev_t *vdev = le2vdev(le, vdev_link);
...
vfs_add_fs(const char *devname, struct fs *fs) {
	cprintf("****************vfs_do_fs  Add a new  fs information \n");
...
vfs_add_dev(const char *devname, struct inode *devnode, bool mountable) {
	cprintf("****************vfs_add_dev  Add a new  device \n");
...
vfs_unmount(const char *devname) {
	cprintf("****************vfs_unmount   Unmount a filesystem\n");
    int ret;
    lock_vdev_list();
    vfs_dev_t *vdev;
    if ((ret = find_mount(devname, &vdev)) != 0) {
...
vfs_unmount_all(void) {
	cprintf("****************vfs_unmount_all   Unmount all  filesystem\n");
    if (!list_empty(&vdev_list)) {
        lock_vdev_list();
        {
            list_entry_t *list = &vdev_list, *le = list;
