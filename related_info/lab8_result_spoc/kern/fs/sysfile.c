...
copy_path(char **to, const char *from) {
	cprintf("******************copy_path - copy path name \n");
...
sysfile_open(const char *__path, uint32_t open_flags) {
	cprintf("******************sysfile_open  open file  \n");
    int ret;
    char *path;
...
sysfile_read(int fd, void *base, size_t len) {
	cprintf("0 ******************sysfile_read  read file  \n");
    struct mm_struct *mm = current->mm;
    if (len == 0) {
        return 0;
    }
...
sysfile_write(int fd, void *base, size_t len) {
	cprintf("0 ******************sysfile_write  write file  \n");
    struct mm_struct *mm = current->mm;
    if (len == 0) {
        return 0;
    }
...
sysfile_seek(int fd, off_t pos, int whence) {
	cprintf("******************sysfile_seek  seek file  \n");
...
sysfile_fstat(int fd, struct stat *__stat) {
	cprintf("******************sysfile_fstat  stat file  \n");
    struct mm_struct *mm = current->mm;
    int ret;
...
sysfile_fsync(int fd) {
	cprintf("******************sysfile_fsync  sync file  \n");
...
sysfile_chdir(const char *__path) {
	cprintf("******************sysfile_chdir  change dir  \n");
    int ret;
    char *path;
    if ((ret = copy_path(&path, __path)) != 0) {
        return ret;
