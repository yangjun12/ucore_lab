...
sfs_sync(struct fs *fs) {
	//cprintf("**************** sfs_sync superblock and freemap in memroy into disk\n ");
...
sfs_get_root(struct fs *fs) {
	//cprintf("**************** sfs_get_root get the root directory inode  from disk \n ");
    struct inode *node;
    int ret;
...
sfs_unmount(struct fs *fs) {
	//cprintf("**************** sfs_unmount  unmount sfs, and free the memorys contain sfs->freemap/sfs_buffer/hash_liskt and sfs itself \n ");
    struct sfs_fs *sfs = fsop_info(fs, sfs);
...
//	cprintf("3 **************** sfs_init_read  init iobuf /");
...
sfs_init_freemap(struct device *dev, struct bitmap *freemap, uint32_t blkno, uint32_t nblks, void *blk_buffer) {
	//cprintf("**************** sfs_init_freemap  get data addr in bitmap /");
    size_t len;
