...
bitmap_translate(struct bitmap *bitmap, uint32_t index, WORD_TYPE **word, WORD_TYPE *mask) {
	//cprintf("*************bitmap_translate   according index, get the related word and mask ");
...
bitmap_test(struct bitmap *bitmap, uint32_t index) {
	//cprintf("*************bitmap_test   according index, get the related value (0 OR 1) in the bitmap");
    WORD_TYPE *word, mask;
...
bitmap_free(struct bitmap *bitmap, uint32_t index) {
	//cprintf("*************bitmap_free   according index, set related bit to 1");
    WORD_TYPE *word, mask;
    bitmap_translate(bitmap, index, &word, &mask);
    assert(!(*word & mask));
    *word |= mask;
...
bitmap_destroy(struct bitmap *bitmap) {
	//cprintf("*************bitmap_destroy   free memory contains bitmap");
    kfree(bitmap->map);
    kfree(bitmap);
}
...
bitmap_getdata(struct bitmap *bitmap, size_t *len_store) {
	//cprintf("*************bitmap_getdata   return the length of bits to len_store");
    if (len_store != NULL) {
